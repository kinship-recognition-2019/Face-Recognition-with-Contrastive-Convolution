...
def main():
    ...
    for iterno in range(args.start_epoch + 1, args.iters + 1):
        ...
        if iterno % 10 == 0:
            ...
            ratios=[]
            save_name = args.save_path + str(args.basemodel) + ' fd' + str(args.idx) + '_'+ str(iterno) + 'checkpoint.pth.tar'
            save_checkpoint(
                {'iterno': iterno,
                'state_dict1': genmodel.state_dict(), 'state_dict2': basemodel.state_dict(),
                'state_dict3': reg_model_kinship.state_dict(), 'state_dict4': idreg_model.state_dict(),
