...
from contrastive_cnn import Contrastive_34Layers
...
def train(args, basemodel, idreg_model, genmodel, reg_model, reg_model_kinship, device, train_loader, optimizer, criterion, criterion1, iteration,alpha,ratios):
    ...
    for batch_idx, (data_1, data_2, c1, c2, target) in enumerate(train_loader):
        ...
        feature_vector1 = org_kernel_1.view(bs,m*n)
        feature_vector2 = org_kernel_1.view(bs,m*n)
        dist = F.pairwise_distance(feature_vector1,feature_vector2,1)
        loss2 = criterion(dist,target)
...
def main():
    ...
    parser.add_argument('--batch_size', type=int, default=50, metavar='N', help='input batch size for training (default: 64)')
    ...
    parser.add_argument('--num_classes', default=1000, type=int,
                        metavar='N', help='number of classes (default: 10574)')
    parser.add_argument('--alpha',default=0.01,type=float,help="the weight of loss2")
    parser.add_argument('--fiw-train-list-path', type=str, default='../dataset/FIW_List/father-daughter/fd_train7.csv',
                        help='path to fiw train list')
    parser.add_argument('--fiw-test-list-path', type=str, default='../dataset/FIW_List/father-daughter/fd_test7.csv',
                        help='path to fiw test list')
    parser.add_argument('--pairs_list_path',type=str,default="list/fd_train7.csv",help='pairs list csv')
    parser.add_argument('--idx', type=int, default=7, help='index of the cross valitation(0 ~ 9)')
    ...
    elif(args.basemodel == "Contrastive_34Layers"):
    ...
    elif(args.basemodel == "seresnet50"):
        print("seresnet50")
        basemodel = seresnet50()
    elif(args.basemodel == "resnet50"):
        print("resnet50")
        basemodel = resnet50()
    elif(args.basemode == "inception_resnet_v2"):
    ...
    # 损失函数
    criterion2 = Contrastive_loss(32.0).to(device)
