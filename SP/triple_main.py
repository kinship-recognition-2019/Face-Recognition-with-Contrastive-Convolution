...
def train(args, basemodel, idreg_model, genmodel, reg_model,  device, train_loader, optimizer, criterion, criterion1, iteration):
    ...
    for batch_idx, (data_1, data_2, data_3) in enumerate(train_loader):
        ...
        loss1 = (loss1+criterion1(SCD, torch.zeros(bs,length).to(device)))/2
...
def main():
    ...
    parser.add_argument('--batch_size', type=int, default=60, metavar='N', help='input batch size for training (default: 64)')
    ...
    parser.add_argument('--start-epoch', default=19000, type=int, metavar='N', help='manual epoch number (useful on restarts)')
    ...
    parser.add_argument('--resume', default='model_kinship19000_checkpoint.pth.tar', type=str, metavar='PATH',
    ...
    parser.add_argument('--num_classes', default=1000, type=int,
                        metavar='N', help='number of classes (default: 10574)')
    parser.add_argument('--fiw-train-list-path', type=str, default='/media/zhang/新加卷/pycode/Face-Recognition-with-Contrastive-Convolution-pytorch/dataset/ss_train.csv',
                        help='path to fiw train list')
    parser.add_argument('--fiw-train-triples-list-path', type=str, default='/media/zhang/新加卷/pycode/Face-Recognition-with-Contrastive-Convolution-pytorch/dataset/ss_train_triples.csv',
                        help='path to fiw train list')
    parser.add_argument('--fiw-test-list-path', type=str, default='/media/zhang/新加卷/pycode/Face-Recognition-with-Contrastive-Convolution-pytorch/dataset/ss_test.csv',
    ...
    for iterno in range(args.start_epoch + 1, args.iters + 1):
        ...
        train_dataset = FIWTrainTripleDataset(img_path=args.fiw_img_path, list_path=args.fiw_train_triples_list_path,noofpairs=args.batch_size, transform=transform)
