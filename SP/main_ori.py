...
def train(args, basemodel, idreg_model, genmodel, reg_model, device, train_loader, optimizer, criterion, criterion1,
          iteration):
    genmodel.train()
    reg_model.train()
    idreg_model.train()
...
def main():
    ...
    parser.add_argument('--root_path', default='/data/Saurav/DB/CASIAaligned/', type=str, metavar='PATH',
                        # path of the CASIA dataset
    ...
    test_loader = torch.utils.data.DataLoader(LFWDataset(dir=args.lfw_dir, pairs_path=args.lfw_pairs_path,
                                                         transform=test_transform), batch_size=args.test_batch_size,
    ...
    idreg_model = Identity_Regressor(14 * 512 * 3 * 3, args.num_classes).to(
        device)  # Kernel o/p for Identity recongition
